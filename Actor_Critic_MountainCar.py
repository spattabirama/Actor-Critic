...
class ActorNN(nn.Module):
    def __init__(self, observation_space, action_space):
        ...
        self.layers = nn.Sequential(
            nn.Linear(observation_space, 4),
            nn.ReLU(),
            nn.Linear(4, action_space),
            nn.Softmax(dim=1)
        )
...
if __name__ == '__main__':
    ...
    actor_optim = optim.Adam(actor_NN.parameters(), lr=1)

    critic_NN = CriticNN(env.observation_space.shape[0])
    critic_optim = optim.Adam(critic_NN.parameters(), lr=1)
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            print(state, action, action_prob, log_prob, next_state, r, term, trunc)
            done = term or trunc
            reward += r
